...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(self, env: Any, budget: float, max_hops: int) -> list[AttackCandidate]:
        ...
        while len(cands) < cap:
            ...
            elif fast_row and fast_multipost_n > 1 and fast_multi_enabled:
            ...
            if fast_row and fast_multipost_n > 1 and fast_multi_enabled:
                fast_probe_n += 1
                fast_probe_posts += posts
                if (
                    fast_probe_n >= FAST_ADAPT_PROBES
                    and fast_probe_posts / fast_probe_n < FAST_MIN_AVG_POSTS
                ):
                    # If the fast model is not actually completing the second
                    # post, stop paying the longer prompt/turn cost and fall
                    # back to the V19 single-post fast path.
                    fast_multi_enabled = False
            if fired:
