...
# Fast-row (Gemma-like) template race.  The proven verbose TEMPLATE stays the
# default; these are only adopted when they fire consistently and beat the
# classification latency by a clear margin.  This spends a few live probes to avoid
# blindly switching to a brittle short form.
FAST_TEMPLATE_PROBES = (
    (
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(self, env: Any, budget: float, max_hops: int) -> list[AttackCandidate]:
        ...
        while len(cands) < cap:
            ...
            if (
                split_on
                and not fast_probe_done
                and classify_n >= split_classify_n
                and chosen_template == TEMPLATE
            ):
                fast_probe_done = True
                baseline_latency = max(classify_sum / max(1, classify_n), LAT_FLOOR_S)
                best_template = TEMPLATE
                best_latency = baseline_latency
                for probe_template in FAST_TEMPLATE_PROBES:
                    fired_count = 0
                    elapsed_sum = 0.0
                    for _ in range(max(1, FAST_PROBE_REPS)):
                        next_wall = slowest * SLOWEST_MULT
                        if (replay_cost + next_wall >= replay_cap) or (
                            time.monotonic() + next_wall >= wall_deadline
                        ):
                            break
                        probe_msg = _msg(idx, probe_template)
                        idx += 1
                        t1 = time.monotonic()
                        try:
                            env.reset()
                            env.interact(probe_msg, max_tool_hops=hops)
                            probe_fired = self._fired(env)
                        except Exception:
                            probe_fired = False
                        probe_elapsed = time.monotonic() - t1
                        slowest = max(slowest, probe_elapsed, LAT_FLOOR_S)
                        elapsed_sum += probe_elapsed
                        if probe_fired:
                            fired_count += 1
                            cands.append(AttackCandidate.from_messages((probe_msg,)))
                            replay_cost += probe_elapsed
                    if fired_count >= max(1, FAST_PROBE_REPS):
                        avg_latency = elapsed_sum / max(1, fired_count)
                        if avg_latency * FAST_MIN_SPEEDUP < best_latency:
                            best_latency = avg_latency
                            best_template = probe_template
